...
// Get products with optional filtering (public access) - FIXED CATEGORY FILTERING
router.get('/', optionalAuth, async (req, res) => {
  try {
    ...
    // FIXED: More robust category filtering
    if (category) {
      // Normalize category to match database exactly
      ...
      console.log('🔍 PUBLIC: Category filter applied:', query.category);
    }
    ...
    console.log('🔍 PUBLIC: Final query:', JSON.stringify(query, null, 2));
    
    const products = await Product.find(query).sort({ createdAt: -1 });
    console.log('📦 PUBLIC: Products found:', products.length);
    
    // Enhanced category breakdown logging
    if (products.length > 0) {
      const menCount = products.filter(p => p.category === 'Men').length;
      const womenCount = products.filter(p => p.category === 'Women').length;
      const otherCount = products.filter(p => p.category !== 'Men' && p.category !== 'Women').length;
      console.log(`👔 PUBLIC: Men's products: ${menCount}, 👗 Women's products: ${womenCount}, ❓ Other: ${otherCount}`);
      
      // Log first few products for debugging
      products.slice(0, 3).forEach(p => {
        console.log(`📋 Sample product: "${p.title}" - Category: "${p.category}" - ID: ${p._id}`);
      });
    }
...
// Create product (restricted to authenticated users) - FIXED CATEGORY VALIDATION
router.post('/', verifyToken, async (req, res) => {
  try {
    ...
    // FIXED: Normalize and validate category
    const normalizedCategory = category.trim();
    let validCategory;
    if (normalizedCategory.toLowerCase() === 'men') {
      validCategory = 'Men';
    ...
    const productData = {
      ...
      style: style.trim(),
      category: validCategory // Use normalized category
    };
...
// Update product (restricted to authenticated users) - FIXED CATEGORY VALIDATION
router.put('/:id', verifyToken, async (req, res) => {
  try {
    ...
    // FIXED: Normalize and validate category if provided
    let validCategory;
    ...
    if (validCategory !== undefined) updateData.category = validCategory; // Use normalized category

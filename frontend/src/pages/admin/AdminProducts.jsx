// Enhanced AdminProducts.js with comprehensive debugging and fixed category handling
...
const AdminProducts = () => {
  ...
  const fetchProducts = async (retryCount = 0) => {
    ...
    try {
      ...
      if (response.ok) {
        // FIXED: Validate and clean product data
        const cleanedProducts = (data || []).map(product => ({
          ...
          length: Array.isArray(product.length) ? product.length : []
        }));
        ...
        const menProducts = cleanedProducts.filter(p => p.category === 'Men');
        const womenProducts = cleanedProducts.filter(p => p.category === 'Women');
        const otherProducts = cleanedProducts.filter(p => p.category !== 'Men' && p.category !== 'Women');
        
        console.log('👔 ADMIN: Men\'s products:', menProducts.length);
        console.log('👗 ADMIN: Women\'s products:', womenProducts.length);
        if (otherProducts.length > 0) {
          console.warn('⚠️ ADMIN: Products with invalid categories:', otherProducts.length);
          otherProducts.forEach(p => console.warn('⚠️ Invalid category product:', {
            id: p._id,
            title: p.title,
            category: p.category
          }));
        ...
        cleanedProducts.slice(0, 3).forEach(p => {
  ...
  const handleAddProduct = async (e) => {
    ...
    if (!newProduct.title || !newProduct.price || !newProduct.brand || !newProduct.material || !newProduct.style) {
    ...
    try {
      ...
      // FIXED: Ensure category is properly formatted before sending
      const productToSend = {
        ...newProduct,
        category: newProduct.category === 'men' ? 'Men' : 
                 newProduct.category === 'women' ? 'Women' : 
                 newProduct.category
      };
      
      console.log('📦 ADMIN: Request body:', JSON.stringify(productToSend, null, 2));
      ...
      if (response.ok) {
        ...
        // FIXED: Update local state immediately with cleaned data
        const cleanedProduct = {
          ...data,
          category: data.category === 'men' ? 'Men' : 
                   data.category === 'women' ? 'Women' : 
                   data.category
        };
        
        setProducts(prevProducts => [...prevProducts, cleanedProduct]);
        setNewProduct({
          title: '',
          price: '',
          imageSrc: '',
        ...
        setSuccessMessage(`Product "${cleanedProduct.title}" added successfully! It will appear on the ${cleanedProduct.category}'s page.`);
        setError('');
        
        // Enhanced event broadcasting
        console.log('📢 ADMIN: Broadcasting productAdded event:', { product: cleanedProduct, category: cleanedProduct.category });
        const event = new CustomEvent('productAdded', { 
          detail: { product: cleanedProduct, category: cleanedProduct.category } 
        });
        window.dispatchEvent(event);
        console.log('📢 ADMIN: Event dispatched successfully');
  ...
  const handleUpdateProduct = async (e) => {
    ...
    try {
      ...
      const productToSend = {
        ...editingProduct,
        category: editingProduct.category === 'men' ? 'Men' : 
                 editingProduct.category === 'women' ? 'Women' : 
      ...
      if (response.ok) {
        ...
        // FIXED: Update local state with cleaned data
        const cleanedProduct = {
          ...data,
          category: data.category === 'men' ? 'Men' : 
                   data.category === 'women' ? 'Women' : 
        ...
        setProducts(products.map(product => 
          product._id === editingProduct._id ? cleanedProduct : product
        ));
        setEditingProduct(null);
        setSuccessMessage(`Product "${cleanedProduct.title}" updated successfully!`);
        setError('');
        ...
        <div className="p-6">
          ...
          <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
              ...
              <div>
                <span className="font-medium">Invalid Category:</span> {products.filter(p => p.category !== 'Men' && p.category !== 'Women').length}
              </div>
            </div>
